...
public class Sort extends Iterator implements GlobalConst
{
  ...
  while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
      ...
      if (sortFldType.attrType == AttrType.attrVector100D && Target != null) {
          ...
          double distance = TupleUtils.calculateEuclideanDistance(vector, Target.getValues());
      ...
      if (k > 0 && p_elems_curr_Q > k) {
        // Remove the tuple with the largest distance (for ascending order)
        // or the smallest distance (for descending order)
        pcurr_Q.deq();
        p_elems_curr_Q--;
      }
  }
  
  // Now the queue is full, start writing to file while adding new tuples
  while (true) {
      cur_node = pcurr_Q.deq();
      if (cur_node == null) break; 
      p_elems_curr_Q--;
      
      // Compare based on distance for attrVector100D
      if (sortFldType.attrType == AttrType.attrVector100D && Target != null) {
        int[] lastElemVector = lastElem.getVectorFld(_sort_fld); // Get the vector from lastElem
        double lastElemDistance = TupleUtils.calculateEuclideanDistance(lastElemVector, Target.getValues());
        comp_res = Double.compare(cur_node.distance, lastElemDistance); // Compare distances
      } else {
      ...
      if ((comp_res < 0 && order.tupleOrder == TupleOrder.Ascending) || 
          ...
          o_buf.Put(cur_node.tuple); // Write tuple to output file
      }

      if (k > 0 && p_elems_other_Q > k) {
        pother_Q.deq();
        p_elems_other_Q--;
      }
          ...
          ) throws IOException, SortException
  {
    ...
    throw new SortException(e, "Sort.java: BUFmgr error");
      ...
      for (int i=0; i<_n_pages; i++) bufs[i] = new byte[MAX_SPACE]; // Fixed: use i instead of k
    }

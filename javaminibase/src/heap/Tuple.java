 ...
 throws IOException, InvalidTypeException, InvalidTupleSizeException		
{
 ...
 switch(types[numFlds -1].attrType) {
   ...
   case AttrType.attrString:
     ...
     break;
  case AttrType.attrVector100D:
     incr = (short) (100 * 4); // 100 integers, 4 bytes each
     break;
  ...
  for (i=0; i< fldCnt-1; i++)
   ...
   case AttrType.attrString:
     ...
     try {
         ...
         System.out.print("Error: Field number out of bound");
     }
     break;  
 ...
 *
 * @param fldNo the field number
 * @return the converted 100D vector as an int array
 * @exception IOException I/O errors
 * @exception FieldNumberOutOfBoundException Tuple field number out of bound
 */
public int[] getVectorFld(int fldNo) 
    throws IOException, FieldNumberOutOfBoundException {
    if ((fldNo > 0) && (fldNo <= fldCnt)) {
        int[] vector = new int[100];
        int offset = fldOffset[fldNo - 1];
        for (int i = 0; i < 100; i++) {
            vector[i] = Convert.getIntValue(offset, data);
            offset += 4; // Move to the next integer (4 bytes per int)
        }
        return vector;
    } else {
        throw new FieldNumberOutOfBoundException(null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
...
public Tuple setVectorFld(int fldNo, int[] vector) 
    throws IOException, FieldNumberOutOfBoundException {
    if ((fldNo > 0) && (fldNo <= fldCnt)) {
        ...
        int offset = fldOffset[fldNo - 1];
        for (int i = 0; i < 100; i++) {
            Convert.setIntValue(vector[i], offset, data);
            offset += 4; // Move to the next integer (4 bytes per int)
        }
        return this;
    } else {
...
/**
 * Get a field as a Vector100Dtype object.
 *
 * @param fldNo the field number
 * @return the Vector100Dtype representation of the field
 * @throws IOException I/O errors
 * @throws FieldNumberOutOfBoundException field number out of bound
 */
public Vector100Dtype getVector100Dtype(int fldNo) 
    throws IOException, FieldNumberOutOfBoundException {
    int[] vector = getVectorFld(fldNo);
    return new Vector100Dtype(vector);
}

/**
 * Set a field from a Vector100Dtype object.
 *
 * @param fldNo the field number
 * @param value the Vector100Dtype value
 * @return this tuple
 * @throws IOException I/O errors
 * @throws FieldNumberOutOfBoundException field number out of bound
 */
public Tuple setVector100Dtype(int fldNo, Vector100Dtype value) 
    throws IOException, FieldNumberOutOfBoundException {
    return setVectorFld(fldNo, value.getValues());
}
